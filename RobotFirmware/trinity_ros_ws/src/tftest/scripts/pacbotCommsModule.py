...
SERVER_ADDRESS = os.environ.get("BIND_ADDRESS","192.168.0.100")
...
class PacbotCommsModule(rm.ProtoModule):
    ...
    def tick(self):
        ...
        # print(str(state.red_ghost.x))
        # print(state.SerializeToString())
        # bs = state.SerializeToString()
        # gs = str(bs)
        # print(gs)
        # print(gs)
        # gs = 'hhh'
        # print(state.SerializeToString())
        # subprocess.call("rostopic pub -r 10 /testtop std_msgs/String " + gs, shell=True)
        if state != None:
            if (state.mode == 0):
