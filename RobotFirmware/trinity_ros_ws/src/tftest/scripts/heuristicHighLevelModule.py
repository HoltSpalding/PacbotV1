...
class HeuristicHighLevelModule(rm.ProtoModule):
    ...
    def _find_best_target(self, p_loc):
        ...
        best_action = heuristics[0]
        to_return = "stay"
        # stay = heuristics[0]
        for value in heuristics:
            if value != float('inf') and value > best_action:
                best_action = value
                # print(best_action)
                # print("best d...............")
        indices = [i for i, e in enumerate(heuristics) if e == best_action]
        if len(indices) == 2:
            indices = indices[1]
        else:
            indices = indices[0]
        ...
        if indices == 4:
            to_return = "down"
        subprocess.call("rostopic pub -r 10 /best_action std_msgs/String " + to_return, shell=True)
        # print(to_return)
        # print(heuristics)
        mins = []
        min_heur = float('inf')
